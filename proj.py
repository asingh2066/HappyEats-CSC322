...
#visitor session is created after log in
@app.route('/visitor_logged_in')
def visitor_logged_in():
    session['username']= 'visitor'              #session is a dictionery with username its key.. value is nm variable
    return redirect(url_for('homepage_customer'))
...
@app.route('/visitor_signed_up')
def visitor_signed_up():
    session['username']= 'visitor'            #session is a dictionery with username its key.. value is nm variable
    password= '123'
    conn=sqlite3.connect('members.db')
    cur=conn.cursor()
...
def search():
	...
	cur.execute("SELECT username FROM managers WHERE place='Manhattan'")
	var_username_TLY=cur.fetchall()
	cur.execute("SELECT username FROM managers WHERE place='Queens'")
	var_username_KANNUR=cur.fetchall()
	cur.execute("SELECT username FROM managers WHERE place='Brooklyn'")
	var_username_CALICUT=cur.fetchall()
	conn.commit()
	conn.close()

	conn=sqlite3.connect('Manhattan.db')
	cur=conn.cursor()
	for x in var_username_TLY:
		...
		if var:
			for y in var:
				...
				cur_temp.execute("INSERT INTO search(item,def,price,category,place,rest,dish_image) VALUES(?,?,?,?,?,?,?); ",(y[0],y[1],y[2],y[3],'Manhattan',x[0],y[4],))
				conn_temp.commit()
				conn_temp.close()
	conn.close()
	...
	for x in var_username_KANNUR:
		...
		if var:
			for y in var:
				conn_temp=sqlite3.connect('members.db')
				cur_temp=conn_temp.cursor()
				cur_temp.execute("INSERT INTO search(item,def,price,category,place,rest,dish_image) VALUES(?,?,?,?,?,?,?); ",(y[0],y[1],y[2],y[3],'Queens',x[0],y[4],))
				conn_temp.commit()
				conn_temp.close()
	conn.close()
	...
	for x in var_username_CALICUT:
		...
		if var:
			for y in var:
				conn_temp=sqlite3.connect('members.db')
				cur_temp=conn_temp.cursor()
				cur_temp.execute("INSERT INTO search(item,def,price,category,place,rest,dish_image) VALUES(?,?,?,?,?,?,?); ",(y[0],y[1],y[2],y[3],'Brooklyn',x[0],y[4],))
				conn_temp.commit()
				conn_temp.close()
	conn.close()
...
def cartshow():
	...
	if(var1[0]>0):
		...
		for x in var:
			if(x[4]=='Manhattan'):
				conn_temp=sqlite3.connect('Manhattan.db')
			elif(x[4]=='Queens'):
				conn_temp=sqlite3.connect('Queens.db')
			else:
				conn_temp=sqlite3.connect('Brooklyn.db')
			cur_temp=conn_temp.cursor()
...
def admin():
	...
	cur.execute('SELECT * FROM managers WHERE place=?',('Manhattan',))
	var_TLY=cur.fetchall()
	cur.execute('SELECT * FROM managers WHERE place=?',('Queens',))
	var_KANNUR=cur.fetchall()
	cur.execute('SELECT * FROM managers WHERE place=?',('Brooklyn',))
